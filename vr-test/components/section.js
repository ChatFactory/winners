        ...
        $.ajax({
            ...
            $.each(response, function (i, item) {
                if(item.images.mobile.src_url !== undefined){
                    if (count < 3) {
                        document.querySelector('#row-one-image-' + i).setAttribute('src', 'https://hackathon-image-proxy.azurewebsites.net/?url=' + item.images.mobile.src_url);
                    ...
                    if (count >= 4 && count < 7) {
                        document.querySelector('#row-two-image-' + i).setAttribute('src', 'https://hackathon-image-proxy.azurewebsites.net/?url=' + item.images.mobile.src_url);
